...
def init_db():
    ...
    CHANGE 7: Ensure session_tokens table exists for persistent login.
    CHANGE 2: Ensure profile_photo column exists on users table.
    """
    try:
        conn = get_connection()
        with conn.cursor() as cur:
            # CHANGE 7: session tokens table for surviving hard refresh
            cur.execute("""
            ...
            """)
            # CHANGE 2: profile photo column (safe to run every time — IF NOT EXISTS)
            cur.execute("""
