...
from rooms.rooms import (
    ...
    is_blacklisted, get_blacklist, unblacklist_user,   # blacklist features
...
def _render_whatsapp_share(room: dict):
    """
    Renders a WhatsApp share button for the room invite link.
    Builds a wa.me link with the invite code pre-filled in the message.
    Also detects ?invite= in URL and auto-fills the join form.
    """
    import urllib.parse
    invite_code = room.get("invite_code", "")
    room_name   = room.get("name", "this room")
    # Build the app URL with ?invite= param so clicking the link auto-fills the code
    try:
    ...
    if base_url:
        ...
        msg = f"Hey! Join me on SankalpRoom in '{room_name}' 🚀\nUse invite code: *{invite_code}*\nOr join directly: {join_url}"
    else:
        msg = f"Hey! Join me on SankalpRoom in '{room_name}' 🚀\nUse invite code: *{invite_code}*"
...
def main():
    # Auto-fill invite code from WhatsApp share link (?invite=CODE in URL)
    try:
    ...
    # If user just logged in and has a pending invite, process it immediately
    if st.session_state.get("pending_invite"):
        ...
        if err == "already_member":
            ...
            st.warning(f"Invite link issue: {err}")
        elif r:
            st.session_state["current_room"] = r["id"]
            st.success(f"Joined **{r['name']}**!")
        st.rerun()
