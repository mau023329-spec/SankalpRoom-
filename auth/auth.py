...
Handles login, register, session, and invite-link flow.
"""
...
def register_user(name: str, email: str, password: str):
    ...
    # Username must be unique (used for login when no email)
    ...
    user_id = execute(
        ...
        (name.strip(), email or None, hash_password(password), secrets.choice(colors)),
    )
    return user_id, None


def login_user(email: str, password: str):
    """Login by email OR by display name (for users who skipped email)."""
    identifier = (email or "").strip()
    pw_hash    = hash_password(password)
    # Try email first, then fallback to name
    user = (
        fetchone("SELECT * FROM users WHERE email = ? AND password_hash = ?", (identifier, pw_hash))
        or fetchone("SELECT * FROM users WHERE name = ? AND password_hash = ?", (identifier, pw_hash))
    )
...
def logout():
    # Note: keep pending_invite so if someone logs out and back in,
    # the invite link still works
    for key in ["user_id","user_name","user_email","user_color",
                "current_room","current_subgroup"]:
...
def render_auth_page():
    # ── Read invite code from URL and stash it ──────────────────────
    # Always re-read from URL on every render so it survives reruns
    params      = st.query_params
    invite_code = (params.get("invite", "") or "").strip().upper()
    if invite_code:
        # Store persistently — survives st.rerun()
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    # Show invite banner if coming from an invite link
    ...
    if pending:
        st.markdown(
            ...
            f'<div style="font-size:0.75rem;color:#64748B;font-family:Inter,sans-serif;">'
            f'Sign in or create an account to join automatically.</div>'
            f'</div></div>',
            unsafe_allow_html=True,
        )
    ...
    with right:
        ...
        with form_col:
            ...
            with tab_in:
                ...
                with st.form("lf"):
                    email = st.text_input("Email or display name", placeholder="you@example.com or YourName")
            ...
            with tab_up:
                ...
                with st.form("rf"):
                    ...
                    email2 = st.text_input("Email (optional)", placeholder="you@example.com (leave blank if you want)")
                    ...
                    st.markdown(
                        ...
                        'margin-top:-0.25rem;">* Required · Without email, log in using your display name.</div>',
                        unsafe_allow_html=True,
                    )
                    if st.form_submit_button("Create account →", type="primary", use_container_width=True):
                        if not all([name, pw2, pw2c]):
                            ...
                            if err:
                                ...
                                # Login using email if provided, else username
