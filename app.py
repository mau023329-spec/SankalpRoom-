...
from rooms.rooms import (
    get_user_rooms, get_room, get_room_by_invite,  get_room_members,
    ...
    update_room, promote_to_admin,
    is_blacklisted, get_blacklist, unblacklist_user,
...
def _render_whatsapp_share(room: dict):
    """
    Renders a WhatsApp share button.
    Message is a clean single link — no invite code shown, no instructions.
    The link lands on the app with ?invite=CODE in the URL which handles everything.
    Requires APP_URL in Streamlit secrets e.g. APP_URL = "https://yourapp.streamlit.app"
    """
    import urllib.parse
    invite_code = room.get("invite_code", "")
    room_name   = _h.escape(room.get("name", "SankalpRoom"))
    try:
        base_url = st.secrets.get("APP_URL", "").strip().rstrip("/")
    except Exception:
        base_url = ""

    if not base_url:
        # Graceful fallback if APP_URL not set — show a warning once
        st.markdown(
            '<div style="color:#F59E0B;font-size:0.7rem;font-family:Inter,sans-serif;">'
            '⚠️ Set APP_URL in secrets to enable sharing.</div>',
            unsafe_allow_html=True,
        )
        return

    join_url = f"{base_url}/?invite={invite_code}"
    # Clean, minimal message — just the link. WhatsApp shows a rich preview automatically.
    msg    = f"Join me in *{room.get('name', 'SankalpRoom')}* on SankalpRoom 👇\n{join_url}"
    ...
    st.markdown(
        ...
        f'padding:0.45rem 0.75rem;text-decoration:none;'
        f'font-family:Inter,sans-serif;font-size:0.78rem;font-weight:600;">'
        f'🟢 Invite via WhatsApp</a>',
        unsafe_allow_html=True,
    )
...
def room_header(room, user_name):
    ...
    col_back, col_name, col_wa = st.columns([1, 5, 2])
    with col_back:
    ...
    with col_wa:
        # WhatsApp invite share button — builds a wa.me link with the invite code
        _render_whatsapp_share(room)
...
def _member_avatar_html(m: dict, size: int = 30) -> str:
    """Return an <img> if the member has a profile photo, else initials circle."""
    photo = m.get("profile_photo") or ""
    if photo:
        return (
            f'<img src="{photo}" title="{_h.escape(m["name"])}" '
            f'style="width:{size}px;height:{size}px;border-radius:50%;object-fit:cover;'
            f'border:2px solid #080A0F;margin-right:-6px;display:inline-block;" />'
        )
    c   = m.get("avatar_color", "#6366F1")
    ini = _h.escape("".join(w[0].upper() for w in m["name"].split()[:2]))
    return (
        f'<div title="{_h.escape(m["name"])}" style="width:{size}px;height:{size}px;'
        f'border-radius:50%;background:{c};color:white;display:inline-flex;'
        f'align-items:center;justify-content:center;font-size:{int(size*0.35)}px;'
        f'font-weight:700;border:2px solid #080A0F;margin-right:-6px;">{ini}</div>'
    )
...
def _render_chat_bubble(msg: dict) -> str:
    """
    Fully custom chat bubble HTML.
    Shows profile photo if set, otherwise a coloured initials circle.
    No st.chat_message() — eliminates Streamlit's default red avatar entirely.
    """
    is_ai   = bool(msg.get("is_ai"))
    who     = "SankalpAI" if is_ai else msg.get("user_name", "")
    content = _h.escape(msg.get("content", ""))
    time_s  = str(msg.get("created_at", ""))[:16]
    color   = msg.get("avatar_color", "#6366F1")
    photo   = msg.get("user_photo") or ""

    # Avatar: photo > initials circle > AI robot emoji
    if is_ai:
        avatar = (
        ...
        ini = _h.escape("".join(w[0].upper() for w in who.split()[:2]))
        if photo:
            avatar = (
                f'<img src="{photo}" style="width:32px;height:32px;border-radius:50%;'
                f'flex-shrink:0;object-fit:cover;border:2px solid #1C2030;" />'
            )
        else:
            avatar = (
                ...
                f'justify-content:center;font-size:0.65rem;font-weight:700;">{ini}</div>'
            )
        name_color = "#CBD5E1"

    return (
        f'<div style="display:flex;align-items:flex-start;gap:0.6rem;'
        f'margin-bottom:0.6rem;padding:0.6rem 0.75rem;'
        f'background:#0C0E14;border:1px solid #1C2030;border-radius:10px;">'
        f'{avatar}'
        f'<div style="flex:1;min-width:0;">'
        f'<div style="margin-bottom:0.2rem;">'
        f'<span style="font-family:Space Grotesk,sans-serif;font-weight:600;'
        f'font-size:0.82rem;color:{name_color};">{_h.escape(who)}</span> '
        f'<span style="color:#334155;font-size:0.68rem;font-family:Inter,sans-serif;">{time_s}</span>'
        f'</div>'
        f'<div style="color:#CBD5E1;font-size:0.875rem;font-family:Inter,sans-serif;'
        f'line-height:1.5;word-break:break-word;">{content}</div>'
        f'</div></div>'
    )
...
def render_room_chat(room_id, user_id, messages):
    ...
    # ── Unread divider: remember the last message ID seen when the user opened the room ──
    # Store it only once per room visit — new messages sent after opening are "new"
    last_seen_key = f"_last_seen_room_{room_id}_{user_id}"
    if last_seen_key not in st.session_state and messages:
        # messages are DESC order — first item is the newest at time of load
        st.session_state[last_seen_key] = messages[0]["id"]

    last_seen_id = st.session_state.get(last_seen_key)
    ...
    with box:
        ...
        # Build HTML, inserting the UNREAD divider at the right position
        html_msgs  = ""
        divider_shown = False
        for msg in shown:
            # Insert divider before the first message that is newer than last_seen_id
            if (
                not divider_shown
                and last_seen_id is not None
                and msg["id"] > last_seen_id
            ):
                html_msgs += (
                    '<div style="display:flex;align-items:center;gap:0.5rem;'
                    'margin:0.75rem 0;">'
                    '<div style="flex:1;height:1px;background:#6366F1;opacity:0.35;"></div>'
                    '<span style="color:#6366F1;font-size:0.68rem;font-weight:600;'
                    'font-family:Inter,sans-serif;white-space:nowrap;'
                    'letter-spacing:0.06em;text-transform:uppercase;">UNREAD</span>'
                    '<div style="flex:1;height:1px;background:#6366F1;opacity:0.35;"></div>'
                    '</div>'
                )
                divider_shown = True
            html_msgs += _render_chat_bubble(msg)
        if html_msgs:
...
def render_room_settings(room_id, user_id, members):
    ...
    for m in members:
        ...
        with col_av:
            # Show profile photo if available, otherwise colour circle
            photo = m.get("profile_photo") or ""
            ini   = _h.escape("".join(w[0].upper() for w in m["name"].split()[:2]))
            if photo:
                av_html = (
                    f'<img src="{photo}" style="width:32px;height:32px;border-radius:50%;'
                    f'object-fit:cover;border:2px solid #1C2030;margin-top:4px;" />'
                )
            else:
                av_html = (
    ...
    st.markdown(
        '<div style="font-family:Space Grotesk,sans-serif;font-size:0.85rem;font-weight:600;'
        'color:#F1F5F9;margin-bottom:0.5rem;">🚫 Banned Members</div>'
        '<div style="color:#64748B;font-size:0.76rem;font-family:Inter,sans-serif;margin-bottom:0.75rem;">'
        'Banned users are blocked by their user ID — changing username won\'t bypass this.</div>',
        unsafe_allow_html=True,
    )
    blacklist = get_blacklist(room_id)
    if not blacklist:
        st.markdown(
            '<div style="color:#334155;font-size:0.8rem;font-family:Inter,sans-serif;'
            'padding:0.5rem 0;">No banned members.</div>',
            unsafe_allow_html=True,
        )
    else:
        for entry in blacklist:
            col_name, col_date, col_unban = st.columns([3, 2, 2])
            with col_name:
                st.markdown(
                    f'<div style="font-family:Inter,sans-serif;font-size:0.83rem;'
                    f'color:#CBD5E1;padding:0.4rem 0;">'
                    f'🚫 {_h.escape(entry["user_name"])}</div>',
                    unsafe_allow_html=True,
                )
            with col_date:
                banned_at = str(entry.get("banned_at", ""))[:10]
                st.markdown(
                    f'<div style="color:#475569;font-size:0.72rem;'
                    f'font-family:Inter,sans-serif;padding:0.5rem 0;">{banned_at}</div>',
                    unsafe_allow_html=True,
                )
            with col_unban:
                if st.button("Unban", key=f"unban_{entry['user_id']}_{room_id}",
                             use_container_width=True):
                    ok, err = unblacklist_user(room_id, entry["user_id"], user_id)
                    if ok:
                        st.success(f"{entry['user_name']} has been unbanned.")
                        st.rerun()
                    else:
                        st.error(err)
...
def render_sg_chat(sg_id, user_id, messages):
    ...
    # ── Unread divider: remember the last message ID seen when user opened the subgroup ──
    last_seen_key = f"_last_seen_sg_{sg_id}_{user_id}"
    if last_seen_key not in st.session_state and messages:
        st.session_state[last_seen_key] = messages[0]["id"]
...
def main():
    # ── Detect ?invite= from WhatsApp link and store in session ──────────────
    try:
        url_invite = st.query_params.get("invite", "")
        if url_invite and "pending_invite" not in st.session_state:
            st.session_state["pending_invite"] = url_invite.upper()
            # Also fetch room name now so auth page can show the banner
            invited_room = get_room_by_invite(url_invite.upper())
            if invited_room:
                st.session_state["pending_invite_room_name"] = invited_room["name"]
    except Exception:
        pass

    if not is_logged_in():
        # Pass the invited room name so auth page can show the invite banner
        invited_room_name = st.session_state.get("pending_invite_room_name", "")
        render_auth_page(invited_room_name=invited_room_name)
        return

    user_id   = st.session_state["user_id"]
    user_name = st.session_state["user_name"]

    # ── Process pending invite immediately after login/signup ─────────────────
    if st.session_state.get("pending_invite"):
        invite_code = st.session_state.pop("pending_invite")
        st.session_state.pop("pending_invite_room_name", None)
        r, err = join_room(invite_code, user_id)
        if err == "already_member":
            ...
            st.session_state["current_room"] = r["id"]
            st.success(f"🎉 You've joined **{r['name']}**!")
        st.rerun()
