...
def _e(text) -> str:
    """Escape user-supplied text before embedding in HTML f-strings."""
...
def _render_inbox_panel(user_id: int):
    ...
    div[data-testid="stButton"] button.inbox-card {
    ...
    div[data-testid="stButton"] button.inbox-card:hover {
            ...
            for u in results:
                ...
                with av_col:
                    ...
                    <div style="width:36px;height:36px;border-radius:50%;
                        ...
                        font-family:'Space Grotesk',sans-serif;margin-top:6px;">{_e(ini)}</div>
                    """, unsafe_allow_html=True)
                with name_col:
    ...
    for conv in inbox:
        ...
        preview  = _e(raw_msg[:40] + ("…" if len(raw_msg) > 40 else ""))
        ...
        # Render the whole card as a Streamlit button — full-width, styled to look
        # like the HTML card. This is the only reliable way to get a tappable area
        # on both mobile and desktop without invalid widget params.
        btn_label = (
            f"{'🔵 ' if unread else ''}{conv['partner_name']}\n"
            f"{raw_msg[:40]}{'…' if len(raw_msg) > 40 else ''}"
        )
        if st.button(btn_label, key=f"inbox_{pid}", use_container_width=True):
        ...
        <div style="display:flex;align-items:center;gap:0.625rem;padding:0.25rem 0 0.75rem;">
            ...
            <div>
                <div style="font-family:'Space Grotesk',sans-serif;font-size:0.92rem;
                    font-weight:700;color:#F1F5F9;line-height:1.2;">{_e(partner['name'])}</div>
        ...
        if not messages:
            ...
            <div style="text-align:center;padding:3rem 1rem;">
                ...
                <div style="font-family:'Space Grotesk',sans-serif;font-size:0.88rem;
                    ...
                    Start a conversation with {_e(partner['name'])}</div>
            </div>
            """, unsafe_allow_html=True)
