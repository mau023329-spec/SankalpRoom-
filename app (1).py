...
# ══════════════════════════════════════════════════════════════════
# GLOBAL CSS — self-contained, no external dependencies
# ══════════════════════════════════════════════════════════════════
...
.stTextInput > label, .stTextArea > label, .stSelectbox > label, .stMultiSelect > label {
    ...
    letter-spacing: 0.06em !important; font-family: 'Inter', sans-serif !important;
}
...
.stButton > button[kind="primary"]:hover {
    background: #4F52D9 !important; box-shadow: 0 4px 20px rgba(99,102,241,0.4) !important;
}
...
# ══════════════════════════════════════════════════════════════════
# UI HELPERS — all inline, no external imports
# ══════════════════════════════════════════════════════════════════
        ...
        with cols[i]:
            ...
            <div style="background:#0C0E14;border:1px solid #1C2030;border-radius:8px;
                ...
                <div style="position:absolute;top:0;left:0;right:0;height:2px;
                    background:{s.get('color','#6366F1')};"></div>
                <div style="font-family:'Space Grotesk',sans-serif;font-size:1.6rem;
                    font-weight:700;color:{s.get('color','#6366F1')};line-height:1;
                    margin-bottom:0.3rem;">{s['value']}</div>
                <div style="color:#475569;font-size:0.72rem;font-weight:500;
                    text-transform:uppercase;letter-spacing:0.06em;
                    font-family:'Inter',sans-serif;">{s['label']}</div>
            </div>
            """, unsafe_allow_html=True)
...
def build_sidebar(user_id):
    with st.sidebar:
        ...
        st.markdown('<div style="color:#334155;font-size:0.68rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;padding:0.5rem 0.25rem 0.25rem;font-family:Inter,sans-serif;">Rooms</div>', unsafe_allow_html=True)
        ...
        if cur_room:
            ...
            if sgs:
                st.markdown('<div style="color:#334155;font-size:0.68rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;padding:0.75rem 0.25rem 0.25rem;font-family:Inter,sans-serif;">Subgroups</div>', unsafe_allow_html=True)
                for sg in sgs:
...
def render_dashboard(user_id, user_name):
    ...
    <div style="padding:0.5rem 0 1.5rem;">
        ...
        <div style="color:#334155;font-size:0.85rem;font-family:'Inter',sans-serif;">
            Pick a room to continue, or create a new one.
        </div>
    </div>
    """, unsafe_allow_html=True)
            ...
            <div style="background:#0C0E14;border:1px solid #1C2030;border-radius:8px;
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.5rem;">
                    <div style="font-family:'Space Grotesk',sans-serif;font-size:0.92rem;
                        font-weight:700;color:#E2E8F0;letter-spacing:-0.01em;">{room['name']}</div>
                    <span style="background:{role_color}18;color:{role_color};
                        border:1px solid {role_color}25;border-radius:4px;
                        padding:1px 7px;font-size:0.65rem;font-weight:600;
                        font-family:'Inter',sans-serif;text-transform:uppercase;
                        letter-spacing:0.06em;">{role_text}</span>
                </div>
                <div style="color:#334155;font-size:0.78rem;font-family:'Inter',sans-serif;
                    margin-bottom:0.75rem;min-height:1.1rem;">
    ...
    with col_l:
        ...
        <div style="margin-bottom:0.75rem;">
            <div style="font-family:'Space Grotesk',sans-serif;font-size:1.3rem;
                font-weight:700;color:#F1F5F9;letter-spacing:-0.02em;margin-bottom:0.2rem;">
                {room['name']}</div>
    ...
    with box:
        ...
        for msg in shown:
            ...
            with st.chat_message("assistant" if is_ai else "user"):
                ...
                name_color = "#818CF8" if is_ai else "#CBD5E1"
                st.markdown(f'<span style="font-family:Space Grotesk,sans-serif;font-weight:600;font-size:0.82rem;color:{name_color};">{who}</span> <span style="color:#1E293B;font-size:0.72rem;">{str(msg.get("created_at",""))[:16]}</span>', unsafe_allow_html=True)
        ...
        with cols[i % 3]:
            ...
            <div style="background:#0C0E14;border:1px solid #1C2030;border-left:3px solid {color};
                ...
                <div style="font-family:'Space Grotesk',sans-serif;font-size:0.88rem;
                    font-weight:700;color:#E2E8F0;margin-bottom:0.3rem;">{sg['name']}</div>
