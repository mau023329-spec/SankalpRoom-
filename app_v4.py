...
.main-nav .stTabs [data-baseweb="tab"] {
    ...
    border-radius: 8px !important; white-space: nowrap !important; flex: 1 !important;
}
...
def room_header(room, user_name):
    ...
    with col_name:
        st.markdown(
            ...
            f'font-weight:700;color:#F1F5F9;padding:0.5rem 0;">{_h.escape(room["name"])}</div>',
            unsafe_allow_html=True,
        )
...
def sg_header(sg, room, user_name):
    ...
    with col_name:
        ...
        st.markdown(
            ...
            f'<div style="font-family:Space Grotesk,sans-serif;font-size:0.75rem;color:#475569;">'
...
def room_card_ui(room):
    ...
    st.markdown(
        ...
        f'color:#E2E8F0;flex:1;margin-right:0.5rem;">{_h.escape(room["name"])}</div>'
...
def sg_card_ui(sg, is_member):
    ...
    st.markdown(
        ...
        f'color:#E2E8F0;margin-bottom:0.25rem;">{_h.escape(sg["name"])}</div>'
...
def render_main_nav(user_id, user_name):
    ...
    with t_home:    render_home_tab(user_id, user_name)
    with t_rooms:   render_rooms_tab(user_id)
    with t_msgs:    render_dm_page(user_id)
    with t_account: render_account_tab(user_id, user_name)
