...
CHANGE 1: Multiple API Keys (up to 7) with automatic rotation on failure/rate-limit.
Keys are read from st.secrets as GROQ_API_KEY, GROQ_API_KEY_2 … GROQ_API_KEY_7.
"""
...
# ── CHANGE 1: Multi-key helpers ───────────────────────────────────────────────

def _get_all_api_keys() -> list:
    """
    Collect up to 7 Groq API keys from secrets / env vars.
    Primary key: GROQ_API_KEY
    Additional:  GROQ_API_KEY_2 … GROQ_API_KEY_7
    """
    keys = []
    primary = st.secrets.get("GROQ_API_KEY", "") or os.environ.get("GROQ_API_KEY", "")
    if primary:
        keys.append(primary.strip())
    for i in range(2, 8):
        k = st.secrets.get(f"GROQ_API_KEY_{i}", "") or os.environ.get(f"GROQ_API_KEY_{i}", "")
        if k and k.strip():
            keys.append(k.strip())
    return keys
...
def _is_configured() -> bool:
    return bool(_get_all_api_keys())


# ── CHANGE 1: Core call with automatic key rotation on rate-limit / auth error ─

def _call_groq(system: str, user_message: str, max_tokens: int = 800) -> str:
    """
    Calls Groq API. On rate-limit or auth error it automatically rotates to the
    next available key and retries — up to len(keys) attempts total.
    """
    keys = _get_all_api_keys()
    if not keys or not GROQ_AVAILABLE:
        return _fallback_response()

    total_keys = len(keys)
    start_idx  = st.session_state.get("_groq_key_idx", 0) % total_keys

    for attempt in range(total_keys):
        current_idx = (start_idx + attempt) % total_keys
        client = Groq(api_key=keys[current_idx])
        try:
            ...
            # Success — persist working key index
            st.session_state["_groq_key_idx"] = current_idx
            return response.choices[0].message.content

        except Exception as e:
            err_str = str(e).lower()
            if any(x in err_str for x in ["rate_limit", "rate limit", "429", "401", "authentication"]):
                continue   # rotate to next key
            return f"⚠️ Groq Error: {str(e)}"

    return "⚠️ All API keys have reached their rate limit. Please try again later."
...
def render_ai_panel(room_id: int, ideas: list, messages: list, subgroups: list):
    ...
    if not _is_configured():
        ...
        # CHANGE 1: Show key count when multiple keys are loaded
        key_count = len(_get_all_api_keys())
        if key_count > 1:
            st.caption(f"🔑 {key_count} API keys loaded — auto-rotation enabled")
