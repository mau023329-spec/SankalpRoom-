...
# ══════════════════════════════════════════════════════════════════
# REALTIME — Supabase polling via background thread
# ══════════════════════════════════════════════════════════════════
# Supabase Realtime websocket requires an async runtime which clashes
# with Streamlit's sync model. The cleanest solution that actually works
# on Streamlit Cloud is a lightweight background thread that polls for
# new messages every 2 seconds and signals the main thread to rerun.
# This gives ~2 second delivery — indistinguishable from "real-time"
# for a chat app, with zero infrastructure beyond what we already have.

_POLL_INTERVAL = 3   # seconds — fast enough, light on DB


def _get_latest_dm_id(user_id: int) -> int:
    row = fetchone(
        "SELECT MAX(id) as max_id FROM direct_messages WHERE sender_id=? OR receiver_id=?",
        (user_id, user_id),
    )
    return row["max_id"] if row and row["max_id"] else 0


def start_realtime_polling(user_id: int):
    """
    Lightweight background poll — only runs while the DM page is open.
    Uses a stop_event so the thread exits cleanly when the user navigates away.
    """
    thread_key = f"_dm_poll_active_{user_id}"
    stop_key   = f"_dm_poll_stop_{user_id}"

    # If a thread is already running for this user, do nothing
    if st.session_state.get(thread_key):
        return

    # Initialise baseline
    if "dm_last_seen_id" not in st.session_state:
        st.session_state["dm_last_seen_id"] = _get_latest_dm_id(user_id)

    stop_event = threading.Event()
    st.session_state[stop_key]   = stop_event
    st.session_state[thread_key] = True

    def _poll():
        while not stop_event.is_set():
            try:
                latest = _get_latest_dm_id(user_id)
                if latest > st.session_state.get("dm_last_seen_id", 0):
                    st.session_state["dm_new_message"] = True
                    st.session_state["dm_last_seen_id"] = latest
            except Exception:
                pass
            stop_event.wait(_POLL_INTERVAL)
        # Thread finished — allow a new one to start next time
...
def stop_realtime_polling(user_id: int):
    """Call this when navigating away from the DM page."""
    stop_key = f"_dm_poll_stop_{user_id}"
    ev = st.session_state.get(stop_key)
    if ev:
        ev.set()
...
def render_dm_page(current_user_id: int):
    start_realtime_polling(current_user_id)
    if st.session_state.pop("dm_new_message", False):
        st.rerun()
...
def _render_conversation(user_id: int, partner_id: int):
    ...
    # Update last seen so polling doesn't fire on messages we just read
    st.session_state["dm_last_seen_id"] = _get_latest_dm_id(user_id)
