...
Mobile-first: tab navigation at top, sidebar on desktop.
...
from rooms.rooms import (
    ...
    delete_room, leave_room, remove_member, delete_subgroup,
...
/* Hide sidebar toggle arrow — the one that was overlapping content */
...
/* ── Inner room tabs — underline style ── */
...
[data-testid="stChatInput"] > div {
    background: #0C0E14 !important; border: 1px solid #1C2030 !important; border-radius: 10px !important;
}
[data-testid="stChatInput"] textarea {
    background: transparent !important; color: #E2E8F0 !important; font-size: 16px !important;
}
...
[data-testid="stExpander"] {
    background: #0C0E14 !important; border: 1px solid #1C2030 !important; border-radius: 8px !important;
}
[data-testid="stExpander"] summary {
    color: #64748B !important; font-size: 0.875rem !important; min-height: 44px !important;
}
...
@media (max-width: 768px) {
    ...
    [data-testid="stHorizontalBlock"] {
        flex-direction: column !important; gap: 0.35rem !important;
    }
...
def mini_header(user_name):
    ...
    with col_brand:
        st.markdown(
            ...
            'font-weight:700;color:#6366F1;">⚡ SankalpRoom</span></div>',
            unsafe_allow_html=True,
        )
    with col_user:
        st.markdown(
            ...
            f'font-family:Space Grotesk,sans-serif;">{initials}</div></div>',
            unsafe_allow_html=True,
        )


def room_header(room, user_name):
    """Compact header for inside a room — back button + room name."""
    ...
    with col_name:
        st.markdown(
            ...
            f'font-weight:700;color:#F1F5F9;padding:0.5rem 0;">{room["name"]}</div>',
            unsafe_allow_html=True,
        )
...
def sg_header(sg, room, user_name):
    ...
    with col_name:
        ...
        st.markdown(
            ...
            f'color:#475569;padding:0.15rem 0 0;">{room["name"] if room else ""}</div>'
            f'<div style="font-family:Space Grotesk,sans-serif;font-size:1rem;'
            f'font-weight:700;color:{color};">{sg["name"]}</div>',
            unsafe_allow_html=True,
        )
...
# ══════════════════════════════════════════════════════════════════
# MAIN NAV TABS  (the pill tab switcher at the top)
# ══════════════════════════════════════════════════════════════════

def render_main_nav(user_id, user_name):
    """Top-level tab slider: Home | Rooms | Messages | Account"""
    inbox        = get_inbox(user_id)
    total_unread = sum(c.get("unread",0) for c in inbox)
    ...
    t_home, t_rooms, t_msgs, t_account = st.tabs(["⌂ Home", "🏠 Rooms", msg_label, "⚙ Account"])
    st.markdown('</div>', unsafe_allow_html=True)

    with t_home:
        render_home_tab(user_id, user_name)

    with t_rooms:
        render_rooms_tab(user_id)

    with t_msgs:
        render_dm_page(user_id)

    with t_account:
        render_account_tab(user_id, user_name)
...
def render_home_tab(user_id, user_name):
    ...
    st.markdown(
        ...
        f'<div style="font-family:Space Grotesk,sans-serif;font-size:1.4rem;font-weight:700;'
        f'color:#F1F5F9;letter-spacing:-0.03em;margin-bottom:0.2rem;">'
        f'Hey, {first} 👋</div>'
...
def render_rooms_tab(user_id):
    ...
    with st.expander("＋ Create new room"):
        with st.form("crf_tab", border=False):
            ...
            rd = st.text_area("Purpose", height=64, placeholder="What's this room for?")
...
def render_account_tab(user_id, user_name):
    ...
    st.markdown(
        ...
        f'background:linear-gradient(135deg,#6366F1,#8B5CF6);'
        f'display:flex;align-items:center;justify-content:center;'
        f'font-size:1.1rem;font-weight:700;color:white;'
        f'font-family:Space Grotesk,sans-serif;">{initials}</div>'
        f'<div>'
        f'<div style="font-family:Space Grotesk,sans-serif;font-size:1rem;font-weight:700;'
        f'color:#F1F5F9;">{user_name}</div>'
        f'<div style="color:#475569;font-size:0.78rem;font-family:Inter,sans-serif;">SankalpRoom member</div>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
    if st.button("Sign out", use_container_width=True):
...
def render_room_view(room_id, user_id, user_name):
    ...
    is_admin = is_room_admin(room_id, user_id)
    tab_labels = ["💬 Chat", "💡 Ideas", "🔬 Groups", "🤖 AI"]
    if is_admin:
        tab_labels.append("⚙ Settings")
    else:
        tab_labels.append("🚪 Leave")
    ...
    with tabs[4]:
        if is_admin:
            render_room_settings(room_id, user_id, members)
        else:
            render_leave_room(room_id, user_id)
...
def render_room_settings(room_id: int, user_id: int, members: list):
    import html as _h
    ...
    for m in members:
        ...
        with col_btn:
            if not is_self and not is_admin:
                if st.button("Remove", key=f"rm_member_{m['id']}", use_container_width=True):
    ...
    st.markdown(
        '<div style="color:#475569;font-size:0.78rem;font-family:Inter,sans-serif;margin-bottom:0.75rem;">'
    ...
    if not st.session_state.get(confirm_key):
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Yes, delete", key=f"confirm_yes_{room_id}",
                         type="primary", use_container_width=True):
                ok, err = delete_room(room_id, user_id)
                if ok:
                    for k in ["current_room","current_subgroup",confirm_key]:
                        st.session_state.pop(k, None)
                    st.rerun()
                else:
                    st.error(err)
        with c2:
            if st.button("Cancel", key=f"confirm_no_{room_id}", use_container_width=True):
                st.session_state.pop(confirm_key, None)
                st.rerun()


def render_leave_room(room_id: int, user_id: int):
    section_hdr("Leave Room")
    st.markdown(
        '<div style="color:#475569;font-size:0.82rem;font-family:Inter,sans-serif;margin-bottom:1rem;">'
        'You will lose access to this room once you leave.</div>',
        unsafe_allow_html=True,
    )
    confirm_key = f"confirm_leave_{room_id}"
    if not st.session_state.get(confirm_key):
        if st.button("Leave this room", key=f"leave_room_{room_id}", use_container_width=True):
            st.session_state[confirm_key] = True
            st.rerun()
    else:
        st.warning("Are you sure you want to leave?")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Yes, leave", key=f"leave_yes_{room_id}",
                         type="primary", use_container_width=True):
...
def render_sg_chat(sg_id, user_id, messages):
    section_hdr("Chat", "/ai <question> for help")
...
def main():
    ...
    # If inside a room or subgroup, show that view directly
    if cur_sg:
