...
FIX 1: profile_photo added to get_inbox(), get_user(), search_users()
       so profile photos appear in the inbox list, conversation header,
       and the sender avatar in each message bubble.
"""
...
def get_conversation(user_a: int, user_b: int, limit: int = 100):
    # FIX 1: also fetch sender's profile_photo so bubbles show real photo
    return fetchall("""
...
def get_inbox(user_id: int):
    # FIX 1: added u.profile_photo AS partner_photo to the SELECT
    return fetchall("""
        WITH last_msgs AS (
            SELECT DISTINCT ON (
...
def search_users(query: str, exclude_id: int):
    q = f"%{query}%"
    # FIX 1: also fetch profile_photo for search results
    return fetchall(
        "SELECT id, name, avatar_color, profile_photo FROM users WHERE id != ? AND (name LIKE ? OR email LIKE ?) LIMIT 10",
        (exclude_id, q, q),
    )


def get_user(user_id: int):
    # FIX 1: also fetch profile_photo for conversation header
    return fetchone("SELECT id, name, avatar_color, profile_photo FROM users WHERE id=?", (user_id,))
...
def _avatar_circle(name: str, color: str, photo: str, size: int = 36) -> str:
    """
    FIX 1: Returns <img> if photo exists, else coloured initials circle.
    Used in inbox cards, conversation header, and message bubbles.
    """
    ini = _e("".join(w[0].upper() for w in name.split()[:2]))
    if photo:
        return (
            f'<img src="{photo}" style="width:{size}px;height:{size}px;'
            f'border-radius:50%;object-fit:cover;flex-shrink:0;'
            f'border:2px solid #1C2030;" />'
        )
    return (
        f'<div style="width:{size}px;height:{size}px;border-radius:50%;'
        f'background:{color};color:white;flex-shrink:0;'
        f'display:flex;align-items:center;justify-content:center;'
        f'font-size:{int(size*0.35)}px;font-weight:700;'
        f'font-family:Space Grotesk,sans-serif;">{ini}</div>'
    )
...
def _render_inbox_panel(user_id: int):
    ...
    if search_q:
        ...
        if results:
            for u in results:
                ...
                with av_col:
                    # FIX 1: show profile photo in search results
                    st.markdown(
                        _avatar_circle(u["name"], u["avatar_color"], u.get("profile_photo") or "", 36),
                        unsafe_allow_html=True,
                    )
                with name_col:
    ...
    for conv in inbox:
        ...
        # FIX 1: use _avatar_circle() — shows profile photo if available
        av_html = _avatar_circle(
            conv["partner_name"],
            conv["partner_color"],
            conv.get("partner_photo") or "",
            36,
        )
...
def _render_conversation(user_id: int, partner_id: int):
    ...
    with c_back:
        if st.button("← Back", key="dm_back_btn"):
            ...
            st.rerun()
    with c_info:
        # FIX 1: conversation header shows profile photo
        av_html = _avatar_circle(
            partner["name"],
            partner["avatar_color"],
            partner.get("profile_photo") or "",
            36,
        )
        st.markdown(
    ...
    with chat_box:
        ...
        for msg in messages:
            ...
            if is_mine:
                ...
                # FIX 1: show sender's profile photo in message bubble
                sender_av = _avatar_circle(
                    msg["sender_name"],
                    msg["sender_color"],
                    msg.get("sender_photo") or "",
                    26,
                )
                st.markdown(
