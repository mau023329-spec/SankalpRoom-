...
def mini_header(user_name):
    ...
    # Pure HTML — no st.columns, never stacks
    st.markdown(
        ...
        f'font-size:0.68rem;font-weight:700;color:white;">{initials}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def room_header(room, user_name):
    initials = "".join(w[0].upper() for w in user_name.split()[:2])
    # Full topbar in HTML — never collapses
    st.markdown(
        f'<div style="display:flex;align-items:center;justify-content:space-between;'
        ...
        f'background:linear-gradient(135deg,#6366F1,#8B5CF6);'
        f'display:flex;align-items:center;justify-content:center;'
        f'font-size:0.65rem;font-weight:700;color:white;">{initials}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def main():
    ...
    pending_invite = st.session_state.pop("pending_invite", None)
    if pending_invite:
        room, err = join_room(pending_invite, user_id)
        if room:
            # Clear the invite param from URL so it doesn't re-trigger
            st.query_params.clear()
            st.session_state["current_room"] = room["id"]
            st.session_state.pop("current_subgroup", None)
            st.success(f"You joined {room['name']}! Welcome 🎉")
            st.rerun()
        elif err and err != "already_member":
