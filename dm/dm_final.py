...
def _get_latest_dm_id(user_id: int) -> int:
    """Get the id of the most recent DM involving this user."""
    row = fetchone(
...
def start_realtime_polling(user_id: int):
    """
    Start a background thread that polls for new DMs every 2 seconds.
    If a new message arrives, sets a flag in session_state so the next
    Streamlit interaction triggers a rerun. Safe to call multiple times —
    only one thread runs per session.
    """
    thread_key = f"_dm_poll_thread_{user_id}"
    if st.session_state.get(thread_key):
        return  # Already running

    # Record the current latest message id as the baseline
    if "dm_last_seen_id" not in st.session_state:
        st.session_state["dm_last_seen_id"] = _get_latest_dm_id(user_id)

    def _poll():
        while st.session_state.get(thread_key):
            try:
...
def render_dm_page(current_user_id: int):
    ...
    # If background thread found a new message, rerun to refresh UI
