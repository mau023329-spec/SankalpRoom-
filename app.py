...
.block-container {
    ...
    padding-bottom: 5rem !important;   /* space for bottom nav on mobile */
...
@media (max-width: 768px) {
    ...
    /* Hide desktop sidebar toggle button — it was overlapping content */
    ...
    /* Also hide sidebar itself — navigation is via bottom nav */
    [data-testid="stSidebar"] {
        display: none !important;
    }
...
# ══════════════════════════════════════════════════════════════════
# BOTTOM NAV (mobile) — injected as fixed HTML
# ══════════════════════════════════════════════════════════════════

def bottom_nav(user_id, active_view, cur_room):
    """Fixed bottom navigation bar — only visible on mobile via CSS."""
    inbox        = get_inbox(user_id)
    total_unread = sum(c.get("unread", 0) for c in inbox)
    badge        = f'<span style="position:absolute;top:2px;right:2px;background:#6366F1;color:white;border-radius:50%;width:16px;height:16px;font-size:9px;display:flex;align-items:center;justify-content:center;font-weight:700;">{total_unread}</span>' if total_unread else ""

    # Determine active tab
    is_home  = not cur_room and active_view != "dm" and active_view != "rooms"
    is_rooms = bool(cur_room) or active_view == "rooms"
    is_dm    = active_view == "dm"

    def nav_item(icon, label, is_active, badge_html=""):
        color  = "#6366F1" if is_active else "#475569"
        weight = "700" if is_active else "400"
        return (
            f'<div style="display:flex;flex-direction:column;align-items:center;'
            f'gap:3px;position:relative;padding:6px 0;">'
            f'{badge_html}'
            f'<span style="font-size:1.4rem;line-height:1;">{icon}</span>'
            f'<span style="font-size:10px;font-weight:{weight};color:{color};'
    ...
    st.markdown(
        ...
        <div id="bottom-nav" style="
            ...
            background:#0C0E14;border-top:1px solid #1C2030;
            padding:6px 0 env(safe-area-inset-bottom,8px);
        ">
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;
                max-width:480px;margin:0 auto;">
                {nav_item("⌂", "Home",     is_home)}
                {nav_item("🏠", "Rooms",   is_rooms)}
                {nav_item("💬", "Messages", is_dm, badge)}
                {nav_item("＋", "New",      False)}
            </div>
        </div>
        <style>
    ...
    # Actual clickable buttons hidden under the visual nav — Streamlit buttons only
    st.markdown('<div style="position:fixed;bottom:0;left:0;right:0;z-index:10000;opacity:0;pointer-events:none;" id="bnav-btns">', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        if st.button("Home", key="bn_home"):
            for k in ["current_room","current_subgroup","active_view"]: st.session_state.pop(k,None)
            st.rerun()
    with c2:
        if st.button("Rooms", key="bn_rooms"):
            st.session_state["active_view"] = "rooms"
            st.session_state.pop("current_subgroup",None)
            st.rerun()
    with c3:
        if st.button("DMs", key="bn_dm"):
            st.session_state["active_view"] = "dm"
            st.session_state.pop("current_room",None)
            st.session_state.pop("current_subgroup",None)
            st.rerun()
    with c4:
        if st.button("New", key="bn_new"):
            st.session_state["active_view"] = "new_room"
            st.rerun()
...
def topbar(user_name, room_name=None, sg_name=None):
    ...
    parts    = ["⚡ SankalpRoom"]
    if room_name: parts.append(room_name)
    if sg_name:   parts.append(sg_name)

    col_brand, col_user = st.columns([5, 1])
    with col_brand:
        st.markdown(
            f'<div style="padding:0.5rem 0 0.25rem;">'
            f'<span style="font-family:Space Grotesk,sans-serif;font-size:1rem;'
            f'font-weight:700;color:#6366F1;">⚡ SankalpRoom</span>'
            + (f'<span style="color:#1E2533;margin:0 0.35rem;">/</span>'
               f'<span style="color:#64748B;font-size:0.82rem;">{room_name}</span>'
               if room_name else "")
            + (f'<span style="color:#1E2533;margin:0 0.35rem;">/</span>'
               f'<span style="color:#818CF8;font-size:0.8rem;">{sg_name}</span>'
               if sg_name else "")
            + f'</div>',
            unsafe_allow_html=True,
        )
    with col_user:
        st.markdown(
            f'<div style="display:flex;justify-content:flex-end;padding:0.4rem 0;">'
            f'<div style="width:34px;height:34px;border-radius:50%;'
            f'background:linear-gradient(135deg,#6366F1,#8B5CF6);'
            f'display:flex;align-items:center;justify-content:center;'
            f'font-size:0.68rem;font-weight:700;color:white;'
            f'font-family:Space Grotesk,sans-serif;flex-shrink:0;">{initials}</div></div>',
            unsafe_allow_html=True,
        )
...
def stat_cards(stats):
    ...
    for i, s in enumerate(stats):
        with cols[i]:
            ...
            st.markdown(
                f'<div style="background:#0C0E14;border:1px solid #1C2030;border-radius:8px;'
                ...
                f'<div style="color:#475569;font-size:0.65rem;font-weight:600;text-transform:uppercase;'
...
def member_avatars(members, user_id=None, clickable=False):
    ...
    for m in members[:6]:
        ...
        html += (f'<div title="{m["name"]}" style="width:30px;height:30px;border-radius:50%;'
    ...
    if len(members) > 6:
        html += (f'<div style="width:30px;height:30px;border-radius:50%;background:#1C2030;'
                 f'color:#64748B;display:inline-flex;align-items:center;justify-content:center;'
    ...
    if clickable and user_id:
        others = [m for m in members if m["id"] != user_id][:3]
        if others:
            st.caption("Quick DM:")
            ...
            for i, m in enumerate(others):
                ini = "".join(w[0].upper() for w in m["name"].split()[:2])
                with cols[i]:
                    if st.button(f"💬 {ini}", key=f"dm_av_{m['id']}", use_container_width=True,
                                 help=f"DM {m['name']}"):
                        open_dm_with(m["id"])
                        st.rerun()
...
def section_hdr(title, subtitle=""):
    sub_html = (f'<div style="color:#475569;font-size:0.76rem;margin-top:0.15rem;'
...
def room_card(room):
    ...
    st.markdown(
        ...
        f'padding:1rem 1rem 0.875rem;margin-bottom:0.4rem;">'
        ...
        f'color:#E2E8F0;flex:1;margin-right:0.5rem;">{room["name"]}</div>'
        ...
        f'</div>'
        f'<div style="color:#334155;font-size:0.76rem;font-family:Inter,sans-serif;'
        f'margin-bottom:0.5rem;">{room.get("description","") or "—"}</div>'
...
def sg_card(sg, is_member):
    color  = sg.get("color","#6366F1")
    joined = ('<span style="color:#22C55E;font-weight:600;font-size:0.7rem;">✓ joined</span>'
              if is_member else "")
    st.markdown(
        ...
        f'color:#E2E8F0;margin-bottom:0.25rem;">{sg["name"]}</div>'
        f'<div style="color:#334155;font-size:0.75rem;font-family:Inter,sans-serif;margin-bottom:0.4rem;">'
        f'{sg.get("description","") or "Execution subgroup"}</div>'
...
# ══════════════════════════════════════════════════════════════════
# MOBILE ROOMS LIST  (shown when active_view == "rooms" on mobile)
# ══════════════════════════════════════════════════════════════════

def render_rooms_list(user_id, user_name):
    topbar(user_name)
    section_hdr("Rooms")

    rooms = get_user_rooms(user_id)
    if not rooms:
        empty_state("🏠", "No rooms yet", "Create or join a room below.")

    for room in rooms:
        room_card(room)
        if st.button("Open →", key=f"rl_open_{room['id']}", use_container_width=True):
            st.session_state["current_room"] = room["id"]
            st.session_state.pop("active_view",None)
            st.rerun()

    st.divider()
    section_hdr("Create or join")
    with st.expander("＋ New room"):
        with st.form("crf_m", border=False):
    ...
    with st.expander("→ Join with invite code"):
        with st.form("jrf_m", border=False):
            ic = st.text_input("Invite code", placeholder="ABC12345")
            if st.form_submit_button("Join", type="primary", use_container_width=True):
                if ic:
                    r, err = join_room(ic, user_id)
...
def render_dashboard(user_id, user_name):
    ...
    st.markdown(
        f'<div style="padding:0 0 1.25rem;">'
        f'<div style="font-family:Space Grotesk,sans-serif;font-size:1.5rem;font-weight:700;'
        f'color:#F1F5F9;letter-spacing:-0.03em;margin-bottom:0.25rem;">'
        f'Good to see you, {first}.</div>'
        f'<div style="color:#334155;font-size:0.85rem;font-family:Inter,sans-serif;">'
        f'Pick a room or use the menu below.</div></div>',
        unsafe_allow_html=True,
    )

    rooms = get_user_rooms(user_id)
    if not rooms:
        empty_state("⚡", "No rooms yet", "Use Rooms → New to create your first room.")
        return
...
def render_room_view(room_id, user_id, user_name):
    ...
    st.markdown(
        ...
        f'<div style="font-family:Space Grotesk,sans-serif;font-size:1.2rem;font-weight:700;'
        f'color:#F1F5F9;letter-spacing:-0.02em;">{room["name"]}</div>'
        f'<div style="color:#334155;font-size:0.8rem;font-family:Inter,sans-serif;margin-top:0.15rem;">'
        f'{room.get("description","") or ""}</div></div>',
        unsafe_allow_html=True,
    )
    ...
    t_chat, t_ideas, t_sgs, t_ai = st.tabs(["💬 Chat", "💡 Ideas", "🔬 Groups", "🤖 AI"])
...
def render_room_chat(room_id, user_id, messages):
    section_hdr("Chat", "/ai <question> for AI")
        ...
        for msg in shown:
            ...
            with st.chat_message("assistant" if is_ai else "user"):
                ...
                st.markdown(
                    ...
                    f'font-size:0.82rem;color:{nc};">{who}</span> '
...
def render_subgroup_view(sg_id, room_id, user_id, user_name):
    ...
    if st.button("← Back to room", key="sg_back"):
        st.session_state.pop("current_subgroup",None)
        st.rerun()

    st.markdown(
        f'<div style="font-family:Space Grotesk,sans-serif;font-size:1.1rem;'
        f'font-weight:700;color:{color};margin-bottom:0.25rem;">{sg["name"]}</div>'
        f'<div style="color:#334155;font-size:0.78rem;font-family:Inter,sans-serif;'
        f'margin-bottom:0.75rem;">{sg.get("description","")}</div>',
        unsafe_allow_html=True,
    )
...
def render_sg_chat(sg_id, user_id, messages):
    section_hdr("Subgroup Chat", "/ai <question> for help")
...
def main():
    ...
    # Render bottom nav (visible only on mobile via CSS)
    bottom_nav(user_id, active_view, cur_room)

    # Route
    if active_view == "dm":
        topbar(user_name)
        render_dm_page(user_id)
    elif active_view in ("rooms", "new_room"):
        render_rooms_list(user_id, user_name)
    elif not cur_room:
