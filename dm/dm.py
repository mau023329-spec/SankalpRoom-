...
def _render_inbox_panel(user_id: int):
    ...
    [data-testid="stVerticalBlock"] [data-testid="stButton"] button[kind="secondary"]:hover {
        color: #7C5CCC !important; background: #F5EFE6 !important;
    }
    </style>
    <div style="font-family:'Space Grotesk',sans-serif;font-size:1rem;
        font-weight:700;color:#1A1714;margin-bottom:0.75rem;">Messages</div>
    ...
    if search_q:
        ...
        if results:
            ...
            st.markdown('<div style="color:#B8A898;font-size:0.82rem;padding:0.5rem 0;">No users found</div>',
                        unsafe_allow_html=True)
        return
    ...
    if not inbox:
        st.markdown("""
        <div style="text-align:center;padding:2.5rem 1rem;border:1px dashed #EDE8E0;
            border-radius:10px;margin-top:0.5rem;">
            <div style="font-size:1.75rem;opacity:0.3;margin-bottom:0.5rem;">💬</div>
            <div style="color:#B8A898;font-size:0.82rem;font-family:Inter,sans-serif;">
    ...
    for conv in inbox:
        ...
        bg        = "#F5EFE6" if is_active else "#FFFBF7"
        border    = "1px solid #7C5CCC" if is_active else "1px solid #EDE8E0"
        dot       = '<div style="position:absolute;top:-2px;right:-2px;width:10px;height:10px;border-radius:50%;background:#7C5CCC;border:2px solid #FAF8F5;"></div>' if unread else ""
        ...
        st.markdown(
            ...
            f'color:{"#1A1714" if unread else "#2D2A26"};">{_e(conv["partner_name"])}</span>'
            f'<span style="color:#B8A898;font-size:0.65rem;font-family:Inter,sans-serif;">{date}</span>'
            f'</div>'
            f'<div style="color:#7C6F5E;font-size:0.73rem;font-family:Inter,sans-serif;'
...
def _render_conversation(user_id: int, partner_id: int):
    ...
    with c_info:
        st.markdown(
            ...
            f'font-weight:700;color:#1A1714;">{_e(partner["name"])}</div>'
            f'<div style="color:#B8A898;font-size:0.7rem;font-family:Inter,sans-serif;">'
    ...
    with chat_box:
        if not messages:
            st.markdown(
                ...
                f'font-weight:600;color:#B8A898;">'
        ...
        for msg in messages:
            ...
            if msg_date != prev_date:
                ...
                st.markdown(
                    ...
                    f'<span style="background:#FFFBF7;border:1px solid #EDE8E0;'
                    f'border-radius:20px;padding:2px 12px;color:#B8A898;'
            ...
            if is_mine:
                st.markdown(
                    ...
                    f'<div style="background:#7C5CCC;color:white;'
                    ...
                    f'<div style="text-align:right;color:#B8A898;font-size:0.65rem;'
                ...
                st.markdown(
                    ...
                    f'<div style="max-width:80%;">'
                    f'<div style="background:#FFFBF7;border:1px solid #EDE8E0;color:#2D2A26;'
                    f'border-radius:14px 14px 14px 2px;padding:0.55rem 0.9rem;'
                    f'font-family:Inter,sans-serif;font-size:0.875rem;line-height:1.45;">'
                    f'{_e(msg["content"])}</div>'
                    f'<div style="color:#B8A898;font-size:0.65rem;margin-top:2px;'
...
def _render_empty_dm():
    ...
    <div style="display:flex;flex-direction:column;align-items:center;
        ...
        <div style="font-family:'Space Grotesk',sans-serif;font-size:0.95rem;
            font-weight:700;color:#B8A898;margin-bottom:0.4rem;">Your messages</div>
        <div style="color:#C8B8A8;font-size:0.82rem;font-family:'Inter',sans-serif;">
