...
def get_conversation(user_a: int, user_b: int, limit: int = 100):
    return fetchall("""
        SELECT dm.*, u.name as sender_name, u.avatar_color as sender_color, u.avatar_url as sender_avatar_url
...
def search_users(query: str, exclude_id: int):
    q = f"%{query}%"
    return fetchall(
        "SELECT id, name, avatar_color, avatar_url FROM users WHERE id != ? AND (name LIKE ? OR email LIKE ?) LIMIT 10",
        (exclude_id, q, q),
    )


def get_user(user_id: int):
    return fetchone("SELECT id, name, avatar_color, avatar_url FROM users WHERE id=?", (user_id,))
...
def _render_inbox_panel(user_id: int):
    ...
    # Inbox header with refresh button
    col_title, col_ref = st.columns([5, 1])
    with col_title:
        st.markdown(
            '<div style="font-family:Space Grotesk,sans-serif;font-size:1rem;'            'font-weight:700;color:#F1F5F9;padding:0.25rem 0 0.5rem;">Messages</div>',
            unsafe_allow_html=True,
        )
    with col_ref:
        if st.button("🔄", key="inbox_refresh", use_container_width=True, help="Refresh inbox"):
            st.rerun()
...
def _render_conversation(user_id: int, partner_id: int):
    ...
    purl = partner.get("avatar_url") or None
    pcol = partner.get("avatar_color", "#6366F1")
    pav  = (
        f'<img src="{purl}" style="width:36px;height:36px;border-radius:50%;object-fit:cover;">'
        if purl else
        f'<div style="width:36px;height:36px;border-radius:50%;flex-shrink:0;'
        f'background:{pcol};color:white;display:flex;align-items:center;'
        f'justify-content:center;font-size:0.65rem;font-weight:700;">{ini}</div>'
    )

    # Header row: back + avatar + name + refresh
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:0.625rem;'
        f'padding:0.5rem 0 0.625rem;border-bottom:1px solid #1C2030;margin-bottom:0.75rem;">'
        + pav +
        f'<div style="flex:1;min-width:0;">'
    ...
    # Back + Refresh buttons in a proper row
    col_back, col_refresh = st.columns([3, 1])
    with col_back:
        if st.button("← Back to inbox", key="dm_back_btn", use_container_width=True):
            st.session_state.pop("dm_partner_id", None)
            st.rerun()
    with col_refresh:
        if st.button("🔄", key="dm_conv_refresh", use_container_width=True, help="Refresh messages"):
            st.rerun()
