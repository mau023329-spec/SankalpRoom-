...
def get_inbox(user_id: int):
    # Postgres-compatible inbox query using CTEs
    return fetchall("""
        WITH last_msgs AS (
            SELECT DISTINCT ON (
...
def _render_inbox_panel(user_id: int):
    st.markdown("""
    <style>
    /* Inbox "Open chat" buttons — collapse them to a thin tap strip under each card */
    ...
    for conv in inbox:
        ...
        # Card: HTML for visual, button underneath for click handling
        dot   = "🔵 " if unread else ""
        short = raw_msg[:35] + ("…" if len(raw_msg) > 35 else "")
        ...
        st.markdown(
            ...
            f'font-weight:{"700" if unread else "600"};color:{"#F1F5F9" if unread else "#CBD5E1"};">'
            f'{dot}{_e(conv["partner_name"])}</span>'
