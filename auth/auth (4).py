...
CHANGE 9: Email is optional. Login accepts username OR email.
INVITE:   render_auth_page() accepts invited_room_name to show invite banner.
"""
...
def render_auth_page(invited_room_name: str = ""):
    """
    INVITE: if invited_room_name is provided, shows a 🎉 banner,
    changes the heading, defaults to "Create account" tab, and
    updates the submit buttons to say "& join room".
    """
    import html as _hl
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    with right:
        ...
        with form_col:
            ...
            # ── 🎉 Invite banner ──────────────────────────────────────────────
            if invited_room_name:
                safe_name = _hl.escape(invited_room_name)
                st.markdown(
                    f'<div style="background:linear-gradient(135deg,#6366F120,#8B5CF620);'
                    f'border:1px solid #6366F140;border-radius:10px;'
                    f'padding:0.875rem 1.25rem;margin-bottom:1.25rem;'
                    f'display:flex;align-items:center;gap:0.75rem;">'
                    f'<span style="font-size:1.5rem;">🎉</span>'
                    f'<div>'
                    f'<div style="font-family:Space Grotesk,sans-serif;font-size:0.9rem;'
                    f'font-weight:700;color:#F1F5F9;">You\'ve been invited!</div>'
                    f'<div style="font-family:Inter,sans-serif;font-size:0.78rem;color:#94A3B8;">'
                    f'Sign up or sign in to join '
                    f'<strong style="color:#818CF8;">{safe_name}</strong>'
                    f' — you\'ll land inside the room automatically.</div>'
                    f'</div></div>',
                    unsafe_allow_html=True,
                )
                st.markdown(
                    ...
                    f'font-weight:700;color:#F1F5F9;letter-spacing:-0.025em;margin-bottom:0.3rem;">'
                    f'Join {safe_name}</div>'
                    f'<div style="color:#475569;font-size:0.82rem;font-family:Inter,sans-serif;">'
                    f'Create an account or sign in — you\'ll be taken straight to the room.</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
                # Default to Create account tab for new users arriving via invite
            ...
            with tab_in:
                ...
                with st.form("lf"):
                    ...
                    btn_label = "Sign in & join room →" if invited_room_name else "Sign in →"
            ...
            with tab_up:
                ...
                with st.form("rf"):
                    ...
                    st.markdown("<div style='height:0.25rem'></div>", unsafe_allow_html=True)
                    btn_label = "Create account & join room →" if invited_room_name else "Create account →"
                    if st.form_submit_button(btn_label, type="primary", use_container_width=True):
